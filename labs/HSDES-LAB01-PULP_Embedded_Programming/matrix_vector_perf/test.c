...
int main()
{
    ...
    // faccio un dot product N (righe) volte; il singolo dot product consiste in M (colonne) MAC
    uint32_t elementary_operations_cnt = M * N;

    printf("gemv() performed %d instructions\n", instr_cnt);
    printf("gemv() took %d cycles\n", cycles_cnt);
    printf("gemv() had a CPI of: %f\n", (float)cycles_cnt / instr_cnt);
    printf("gemv() cycles per elementary operation: %f\n", (float)cycles_cnt / elementary_operations_cnt);
    printf("gemv() instructions per elementary operation: %f\n", (float)instr_cnt / elementary_operations_cnt);
}
